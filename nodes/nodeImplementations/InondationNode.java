	...
	public void handleMessages(sinalgo.nodes.messages.Inbox inbox) {
		...
		while(inbox.hasNext()) {
			...
			if (msg instanceof InondationMessage) {
				...
				Node next;
				for(Edge edg : outgoingConnections)//liste de toutes les links du reseaux
				{
				    next=edg.endNode;//on change le noeud de depart, en noeud d'arrivée
					if(!next.getColor().equals(Color.PINK)){//si la couleur est déja vert, c'est que on lui a déja envoyer le message
					send(walker, next);
